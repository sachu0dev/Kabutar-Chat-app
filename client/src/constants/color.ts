export const orange = "#ea7070";
export const grayColor = "Rgba(247,247,247,1)";
export const lightBlue = "#2694ab";
export const matBlack = "#1c1c1c";
export const bgGradient =
  "linear-gradient(-45deg, rgb(238, 119, 82), rgb(230, 175, 196), rgb(35, 166, 213), rgb(255, 255, 255))";

export const purpleColor = "rgb(75, 12, 192, 1)";
export const purpleColorLight = "rgb(75, 12, 192, 0.5)";
export const purpleColor2 = "rgb(175, 12, 192, 1)";
export const purpleColorLight2 = "rgb(175, 12, 192, 0.5)";
